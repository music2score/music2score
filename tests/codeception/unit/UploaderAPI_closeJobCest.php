...
class UploaderAPI_closeJobCest
{
    ...
    public function tryToTest(UnitTester $I)
    {
        ...
        //both file transfers are successfull but extraction fails
        unset($_FILES);
        $_FILES["file_pdf"]["name"]="right_format.pdf";
        $_FILES["file_png"]["name"]="right_format.zip";
        $_FILES["file_pdf"]["tmp_name"]="right_format.pdf";
        ...
        $Uploader_API = Stub::make(Uploader_API::class, ['jobno' => 4,'movefile' => function($x,$y){ return true; }, 'extract' => function($ext){ return false; }]);
        $I->assertFalse($Uploader_API->closeJob($db,$_FILES));
        $I->seeInDatabase('jobs', ['filename' => './some_other_file3.mid','completed' => 0]);
        ...
        //both file transfers are successfull and extraction succeeds
        unset($_FILES);
        $_FILES["file_pdf"]["name"]="right_format.pdf";
        $_FILES["file_png"]["name"]="right_format.zip";
        $_FILES["file_pdf"]["tmp_name"]="right_format.pdf";
        $_FILES["file_png"]["tmp_name"]="right_format.zip";
        $I->haveInDatabase('jobs', array('jobid' => 7, 'filename' => './some_other_file6.mid','userid' => 1,'processing' => 1,'completed' => 0));
        $I->seeInDatabase('jobs', ['filename' => './some_other_file6.mid','completed' => 0]);
        $Uploader_API = Stub::make(Uploader_API::class, ['jobno' => 7,'movefile' => function($x,$y){ return true; }, 'extract' => function($ext){ return true; }]);
        $I->assertTrue($Uploader_API->closeJob($db,$_FILES));
        $I->seeInDatabase('jobs', ['filename' => './some_other_file6.mid','completed' => 1]);
